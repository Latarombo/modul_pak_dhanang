...
if (empty($nis) || empty($nama) || empty($kelas) || empty($alamat) || empty($kota) || empty($jk)) {
    header("Location: edit.php?nis=" . urlencode($nis) . "&error=field_kosong"); exit();
}
if (!in_array($kelas, ['X', 'XI', 'XII'])) { header("Location: edit.php?nis=" . urlencode($nis) . "&error=invalid_kelas"); exit(); }
if (!in_array($jk, ['L', 'P'])) { header("Location: edit.php?nis=" . urlencode($nis) . "&error=invalid_jk"); exit(); }
$tgl = str_pad(trim($_POST['tgl'] ?? ''), 2, '0', STR_PAD_LEFT);
$bln = str_pad(trim($_POST['bln'] ?? ''), 2, '0', STR_PAD_LEFT);
$thn = trim($_POST['thn'] ?? '');
$ttl = $thn . "-" . $bln . "-" . $tgl;
if (!checkdate((int)$bln, (int)$tgl, (int)$thn)) { header("Location: edit.php?nis=" . urlencode($nis) . "&error=invalid_date"); exit(); }
...
if (empty($ekskul_clean)) { header("Location: edit.php?nis=" . urlencode($nis) . "&error=ekskul_kosong"); exit(); }
...
if ($stmt->execute()) { $stmt->close(); header("Location: admin.php?success=updated"); }
else { $stmt->close(); header("Location: edit.php?nis=" . urlencode($nis) . "&error=db_error"); }
exit();
